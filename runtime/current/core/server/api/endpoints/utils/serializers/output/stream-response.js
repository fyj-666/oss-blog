 ...
 */
function createStreamResponse({ source, transform, filename, contentType, missingFilenameMessage = 'Missing export filename', }) {
    return function streamResponse(req, res, next) {
        ...
        if (transform) {
            (0, node_stream_1.pipeline)(source, transform, res, done);
        }
        else {
            (0, node_stream_1.pipeline)(source, res, done);
        }
    };
}
