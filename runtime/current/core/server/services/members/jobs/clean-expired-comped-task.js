...
async function cleanExpiredComped({ db, models, events, logging, sentry, }) {
    const startedAt = Date.now();
    const { deletedSubscriptionCount, updatedMemberCount, updatedMembers, now } = await db.knex.transaction(async (trx) => {
        const expiredCompedRows = await trx('members_products')
        ...
        const compedMembers = await trx('members')
...
// internal context.
async function emitMemberEditedEvent({ member, now, models, events, logging, sentry, }) {
    try {
        const model = await models.Member.findOne({ id: member.id }, { require: true, context: { internal: true } });
        ...
        if (isNotFoundError(err)) {
            logging.warn(`Could not emit member.edited for expired comped member: Member ${member.id} was not found`);
            return false;
        }
        logging.error(err);
...
exports.default = cleanExpiredComped;
