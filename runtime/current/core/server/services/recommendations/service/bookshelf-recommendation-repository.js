...
class BookshelfRecommendationRepository extends bookshelf_repository_1.BookshelfRepository {
    ...
    applyCustomQuery(query, options) {
        ...
        if (options.include?.includes('clickCount') ||
            options.order?.find((o) => o.field === 'clickCount')) {
            query.select((knex) => {
                knex
                    .count('*')
                    .from('recommendation_click_events')
                    .where('recommendation_click_events.recommendation_id', knex.client.raw('recommendations.id'))
                    .as('count__clicks');
            });
        }
        if (options.include?.includes('subscriberCount') ||
            options.order?.find((o) => o.field === 'subscriberCount')) {
            query.select((knex) => {
                knex
                    .count('*')
                    .from('recommendation_subscribe_events')
                    .where('recommendation_subscribe_events.recommendation_id', knex.client.raw('recommendations.id'))
                    .as('count__subscribers');
            });
        }
    }
    toPrimitive(entity) {
