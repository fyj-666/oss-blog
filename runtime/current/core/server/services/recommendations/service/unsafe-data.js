...
class UnsafeData {
        ...
        if (typeof this.data === 'string') {
            ...
            if (isNaN(parsed) || parsed.toString() !== this.data) {
                throw new errors_1.default.ValidationError({
                    message: `${this.field} must be a number, got ${typeof this.data}`,
                });
            }
    ...
    enum(allowedValues) {
        if (!allowedValues.includes(this.data)) {
            throw new errors_1.default.ValidationError({
                message: `${this.field} must be one of ${allowedValues.join(', ')}`,
            });
        }
        return this.data;
    }
    ...
    index(index) {
        ...
        if (index >= arr.length) {
            throw new errors_1.default.ValidationError({
                message: `${this.field} must be an array of length ${index + 1}`,
            });
        }
        return arr[index];
    }
