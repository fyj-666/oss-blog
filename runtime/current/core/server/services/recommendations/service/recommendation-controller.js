...
class RecommendationController {
    ...
    #stringToOrder(str) {
        ...
        for (const [index, part] of parts.entries()) {
            ...
            const fieldData = new unsafe_data_1.UnsafeData(trimmed.split(' ')[0].trim(), {
                field: ['order', index.toString(), 'field'],
            });
            const directionData = new unsafe_data_1.UnsafeData(trimmed.split(' ')[1]?.trim() ?? 'desc', {
                field: ['order', index.toString(), 'direction'],
            });
    ...
    async browse(frame) {
        ...
        const recommendations = await this.service.listRecommendations({
            page,
            limit,
            filter,
            include,
