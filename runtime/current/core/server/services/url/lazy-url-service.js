...
class LazyUrlService {
    ...
    reportedThinResources;
    constructor({ urlUtils = localUtils, findResource, fetchRoutableResources = defaultFetchRoutableResources, }) {
     ...
     */
    async getRoutableResources(type, { columns = [] } = {}) {
    ...
    _degradeThinResource(resource, thin, options) {
    ...
    _missingBaseFields(routerType, resource) {
        const base = this.baseFilters.get(routerType);
        if (!base) {
            return null;
        }
    ...
    _missingRouterFields(config, resource, routerType) {
        if (!config.filter) {
            return null;
        }
        const r = resource;
