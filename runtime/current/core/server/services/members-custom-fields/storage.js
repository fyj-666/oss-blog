...
exports.leavesFor = leavesFor;
exports.leavesToWrite = leavesToWrite;
exports.valueFromLeaves = valueFromLeaves;
exports.valuesFromLeaves = valuesFromLeaves;
...
function leavesFor(value, path = exports.ROOT_PATH) {
    ...
    return Object.entries(value)
        ...
        .flatMap(([key, part]) => leavesFor(part, path === exports.ROOT_PATH ? key : `${path}${SEPARATOR}${key}`));
}
/** A value split into the leaves it sets and the paths it clears. */
function leavesToWrite(value) {
...
function valueFromLeaves(leaves) {
    const root = leaves.find((leaf) => leaf.path === exports.ROOT_PATH);
    if (root) {
    ...
    for (const leaf of leaves) {
        ...
        const parent = segments.reduce((target, segment) => {
...
function valuesFromLeaves(leaves) {
    const byMemberAndField = new Map();
    for (const { member_id: memberId, key, path, value_text: valueText } of leaves) {
        const fields = byMemberAndField.get(memberId) ?? new Map();
    ...
    for (const [memberId, fields] of byMemberAndField) {
        byMember.set(memberId, Object.fromEntries([...fields].map(([key, forField]) => [key, valueFromLeaves(forField)])));
    }
    return byMember;
}
