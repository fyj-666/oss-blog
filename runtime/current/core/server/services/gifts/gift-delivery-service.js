...
exports.GiftDeliveryService = void 0;
...
function wasScheduledDelivery(delivery, gift) {
    return Boolean(delivery.scheduledAt &&
        gift.purchasedAt &&
...
class GiftDeliveryService {
    ...
    async createForCheckout({ giftId, recipientEmail, scheduledAt, }, options = {}) {
        const delivery = {
    ...
    async getRecipientEmailForGift(giftToken, options = {}) {
    ...
    async dispatchForGift({ giftId, }) {
        ...
        DomainEvents.dispatch(send_gift_delivery_event_1.SendGiftDeliveryEvent.create({ deliveryId: delivery.id }));
        return { recipientEmail: delivery.recipientEmail, scheduledAt: delivery.scheduledAt };
    }
    async recoverPending(batchSize = 1000) {
        const result = {
            sentCount: 0,
            skippedCount: 0,
            failedCount: 0,
        ...
        for (let batch = 0; batch < maxBatches; batch++) {
            const now = new Date();
            const staleBefore = new Date(now.getTime() - constants_1.GIFT_DELIVERY_STALE_AFTER_MS);
            ...
            await Promise.all(Array.from({ length: Math.min(RECOVERY_CONCURRENCY, deliveries.length) }, recoverNext));
    ...
    async send(id, giftSnapshot) {
        ...
        if ((persisted || cancelledDuringSend) && result.providerMessageId) {
            try {
                ...
                logging_1.default.error('Failed to schedule gift delivery analytics', err);
            }
        }
    ...
    async notifyBuyerOfDeliveryFailure(providerMessageId) {
        try {
...
exports.GiftDeliveryService = GiftDeliveryService;
function isMailgunRejection(err) {
