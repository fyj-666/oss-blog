...
class EmailAnalyticsJobScheduler {
    ...
    async scheduleRecurringNewslettersJob(skipNewsletterEmailCheck = false) {
        ...
        const emailCount = skipNewsletterEmailCheck
            ? 1
            : Number(await this.#models.Email.where('created_at', '>', moment_1.default.utc().subtract(30, 'days').toDate())
                .where('status', '<>', 'failed')
                .count());
    ...
    async scheduleRecurringAutomationsJob(skipAutomationEmailCheck = false) {
        ...
        const hasAutomatedEmailRecipient = skipAutomationEmailCheck ||
            Boolean(await this.#models.AutomatedEmailRecipient.query()
    ...
    async scheduleRecurringGiftDeliveriesJob(skipGiftDeliveryCheck = false) {
        ...
        const hasGiftDelivery = skipGiftDeliveryCheck ||
            Boolean(this.#models.GiftDelivery &&
                (await this.#models.GiftDelivery.query()
                    .where('email_sent_at', '>', moment_1.default.utc().subtract(30, 'days').toDate())
