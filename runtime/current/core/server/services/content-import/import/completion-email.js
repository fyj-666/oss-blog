...
exports.default = buildCompletionEmail;
const report_1 = __importDefault(require("./report"));
const errors_file_1 = __importDefault(require("./errors-file"));
...
function headingFor(run, counts) {
...
function summaryItems(counts, hasErrorsFile) {
    const items = [
    ...
    return items
        .map(([label, count]) => {
        const attachmentCopy = label === 'Failed' && hasErrorsFile ? ' (see attached errors.csv)' : '';
        return `<li style="margin-bottom: 6px;"><strong>${label}:</strong> ${formatNumber(count)}${attachmentCopy}</li>`;
    })
        .join('');
}
function importTagSlug(runId) {
    // Internal tag slugs replace the leading # with "hash-" in the model layer.
    return slugify(`#Import Run ${runId}`.replace(/^#/, 'hash-'));
}
function importedPostLinks(run, adminUrl) {
    const importedRows = run.rows.filter((row) => row.status === 'created' || row.status === 'updated');
    ...
    const links = previewRows
        ...
        return `<li style="margin-bottom: 6px;"><a href="${escapeHTML(row.url)}" style="color: #3A464C;">${title}</a></li>`;
    })
        .join('');
...
function renderCompletionEmail(run, recipient, adminUrl, hasErrorsFile) {
    ...
    const warningCopy = counts.warningRows
        ...
        : '';
    const failureCopy = run.status === 'failed'
...
function buildCompletionEmail(run, recipient, adminUrl) {
    const counts = countsFor(run);
    const report = (0, report_1.default)(run);
    const errorsFile = (0, errors_file_1.default)(run);
    const attachments = [];
    if (report) {
        attachments.push({
            filename: 'report.csv',
