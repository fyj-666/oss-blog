...
const debug = (0, debug_1.default)('TableImporter');
class TableImporter {
    name;
    knex;
    transaction;
    ...
    fastFakeObjectId() {
        ...
        return `00000000` + node_crypto_1.default.randomBytes(8).toString('hex');
    }
    async #generateData(amount = this.defaultQuantity ?? 0) {
        const data = [];
    ...
    async importForEach(models = [], amount = 0) {
        const data = [];
        ...
        for (const model of models) {
            ...
            if (!Number.isInteger(currentAmount)) {
                currentAmount = Math.floor(currentAmount) + Number((0, random_1.luck)((currentAmount % 1) * 100));
            }
    ...
    async batchInsert(data) {
        // Write to CSV file
        const rootFolder = node_os_1.default.tmpdir();
        const filePath = node_path_1.default.join(rootFolder, `${this.name}.csv`);
        let now = Date.now();
        if (data.length > 5000 && !process.env.DISABLE_FAST_IMPORT) {
            ...
            for (let i = 0; i < data.length; i += batchSize) {
                ...
                for (const obj of slicedData) {
                    ...
                    for (const [key, value] of Object.entries(mutableObj)) {
                        if (typeof value === 'boolean') {
                            ...
                            mutableObj[key] = (0, date_1.toDatabaseDate)(value);
                        }
                        else if (value === null) {
                            mutableObj[key] = '\\N';
                        }
                ...
                const csv = papaparse_1.default.unparse(slicedData, {
                    columns,
                    header: i === 0,
                    newline: '\n',
                });
                await node_fs_1.default.promises.appendFile(filePath, `${i === 0 ? '' : '\n'}${csv}`);
            }
            debug(`${this.name} saved CSV import file in ${Date.now() - now}ms`);
            now = Date.now();
            // Import from CSV file
            const [result] = await this.transaction.raw(`LOAD DATA LOCAL INFILE '${filePath}' INTO TABLE \`${this.name}\` FIELDS TERMINATED BY ',' ENCLOSED BY '"' IGNORE 1 LINES (${Object.keys(data[0])
                .map((d) => '`' + d + '`')
                .join(',')});`);
            if (result.affectedRows !== data.length) {
                ...
                logging_1.default.warn(`CSV import warning: expected ${data.length} imported rows, got ${result.affectedRows}.`);
            }
        }
        else {
            await this.knex
                .batchInsert(this.name, data)
