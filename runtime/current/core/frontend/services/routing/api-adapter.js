...
exports.resolveApiCall = resolveApiCall;
exports.resolveRouteData = resolveRouteData;
exports.resolveResourceRead = resolveResourceRead;
 ...
 */
const RESOURCE_CONFIGS = config_1.QUERY;
/**
...
function unknownResource(resource) {
    return new errors_1.default.IncorrectUsageError({ message: `Unknown route data resource: ${resource}` });
}
// Shorthand names a resource the short way (`tag`), long form the way the API
// does (`tags`). Both land on the same config.
function splitShortForm(shortForm) {
    const [key, slug] = shortForm.split('.');
    return [key, slug];
}
function shortFormConfig(key) {
    return RESOURCE_CONFIGS[key];
}
function longFormConfig(resource) {
    return Object.values(RESOURCE_CONFIGS).find((config) => config?.resource === resource);
}
function resolveShortForm(shortForm) {
...
function resolveLongForm(entry) {
    const config = longFormConfig(entry.resource);
    if (!config) {
        throw unknownResource(entry.resource);
    }
    const options = lodash_1.default.pick(entry, ALLOWED_QUERY_OPTIONS);
    return {
        controller: config.controller,
        type: entry.type,
        resource: config.resource,
        ...
        options: entry.type === 'read' ? lodash_1.default.defaults(options, config.options) : options,
    };
}
/**
 ...
 */
function resolveRouteData(routeData) {
    if (!routeData) {
        return {};
    }
    ...
    return lodash_1.default.mapValues(routeData, (entry) => resolveApiCall(entry));
}
/**
 ...
 */
function resolveResourceRead(entry) {
