...
function getMachinePaymentsService(req) {
 ...
 */
function servePreviewMarkdown(res, entry) {
    const llmsIndexUrl = urlUtils.urlFor({ relativeUrl: '/llms.txt' }, true);
...
function refuseMembersOnlyMarkdown(res) {
 ...
 */
async function servePaidMarkdown(req, res, entry) {
    ...
    const response = await machinePaymentsService.challengeOrFulfill(fetchRequest, {
        ...
        contentLocation,
        renderMarkdown: (paidEntry) => renderEntryMarkdown(paidEntry, { llmsIndexUrl }),
 ...
 */
async function serveMdRequest(req, res, entry) {
    if (!llmsEnabled(req)) {
